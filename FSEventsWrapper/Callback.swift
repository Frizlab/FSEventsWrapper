...
enum Event {
	...
	/* For all event types, the “fromUs” var (whether the event comes from us)
	 * will only be set to a non-nil value if kFSEventStreamCreateFlagMarkSelf is
	 * set.
	 *
	 * The notion corresponds to the kFSEventStreamEventFlagOwnEvent flag.
	 *
	 * Note: If kFSEventStreamCreateFlagIgnoreSelf is set in addition to
	 *       kFSEventStreamCreateFlagMarkSelf, fromUs should always be false.
	 * Note2: The flag does not seem to work for whatever reason… We might wanna
	 *        try disabling SIP and see if flag still does not work ¯\_(ツ)_/¯ */
	...
	replay from the last seen event id when you program relaunch. When receiving
	the “event id wrapped” event, you should retrieve the new UUID for the device
	because it changes when the event id wraps.
	...
	names are in the monitored folder or one of their descendants). There are no
	sure way (AFAICT) to know which is which. From my limited testing, both
	events are sent in the same callback call (you don’t have the information of
	the callback call from FSEventsWrapper) at one event interval, the first one
	being the original name, the second one the new name. */
